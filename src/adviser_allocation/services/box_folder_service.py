...
def get_box_template_path_from_settings() -> str:
    """Get Box template path from environment variable.

    Reads ``BOX_TEMPLATE_PATH`` env var with a sensible default.
    Uses 5-minute cache to avoid redundant lookups.
    """
    ...
    logger.info("Using Box template path from environment variable: %s", path)
...
def refresh_box_template_path_cache() -> None:
    """Clear cached Box template path so the next call re-reads the env var."""
    global _BOX_TEMPLATE_PATH_CACHE, _BOX_TEMPLATE_PATH_CACHE_TIME
